...
const bar = new cliProgress.SingleBar({
  format: colors.blue(
    "Downloading: [{bar}] {percentage}% | {value}/{total} pages | ETA: {eta}s"
  ),
  hideCursor: true,
});
  ...
  rl.question(colors.blue("Give me the code: "), (code) => {
    ...
    axiosins
      ...
      .then(async (response) => {
        ...
        rl.close();
        const images: string[] = await download();
        resolve(images);
      })
      .catch(console.error);
  });
...
const makepdf = () => {
  console.log(colors.blue("Creating pdf..."));
  pdf.pipe(fs.createWriteStream(`./${pubcode}/${pubcode}.pdf`));

  for (var i = 0; images.length > i; i++) {
    var img = pdf.openImage(images[i]);
...
const flow = async () => {
  await Promise.all([first]).then(() => makepdf());
};
